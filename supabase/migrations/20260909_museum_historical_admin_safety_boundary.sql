-- Preserve the original built-in admin identity and UI while preventing the
-- historically exposed credential from receiving destructive database rights.
-- Private legacy admin/super-admin accounts remain privileged.
...
begin
  ...
  select au.* into u from public.app_users au where lower(au.username) = normalized_login limit 1;
  if u.id is not null then
    select c.password_hash into cred_hash from museum_private.credentials c where c.user_key = u.id::text;
  end if;
  ...
  session_role := case
    when normalized_login = 'admin' then 'MUSEUM_VISITOR_ADMIN'
    else app_role
  end;

  raw_token := encode(extensions.gen_random_bytes(32), 'hex');
  insert into museum_private.sessions(token_hash, user_key, role, expires_at)
  values (encode(extensions.digest(raw_token, 'sha256'), 'hex'), u.id::text, session_role, now() + interval '12 hours');
